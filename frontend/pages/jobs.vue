...
<template>
  <div class="flex-1 flex flex-col overflow-y-auto">
    <PageHeader
      title="Jobs"
      :subtitle="`${filteredJobs.length} of ${allJobs.length} jobs`"
    >
      <template #actions>
        <button
    ...
    <div class="flex-1 p-6 space-y-4 max-w-6xl w-full mx-auto">
      ...
      <div class="flex gap-1.5 flex-wrap">
        ...
        >
          ...
          <span
            ...
            :class="activeFilter === f.value ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'"
          >
            {{ f.count }}
          </span>
        </button>
      ...
      <template v-else-if="error">
        <AppCard>
          <ErrorState message="Could not load jobs." :show-retry="true" @retry="() => refresh()" />
        </AppCard>
      </template>

      <template v-else>
        <AppCard v-if="!filteredJobs.length">
          <EmptyState
            title="No jobs found"
            :message="activeFilter === 'all' ? 'Run python main.py --discover-jobs to find opportunities.' : 'No jobs match this filter.'"
          />
        </AppCard>
              ...
              <tbody>
                <tr v-for="job in filteredJobs" :key="job.id">
                  <td>
                    ...
                    <div class="text-xs text-gray-400 mt-0.5 truncate max-w-xs">{{ job.company }}</div>
                  </td>
                  <td class="hidden md:table-cell text-gray-500">{{ job.location || '—' }}</td>
                  <td class="text-right">
                    <span class="font-semibold tabular-nums text-blue-600">{{ job.job_score }}</span>
                  </td>
                  <td>
                  ...
                  <td class="hidden lg:table-cell text-xs text-gray-400">{{ formatDate(job.applied_at) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
